    ...
    attach: function attach(context, settings) {
      $('.custom--carousel', context).once('slick-slider').each(function () {
        ...
        $('.media__image', context).once('slick-slider-image').each(function () {
          ...
          $(this).attr('data-analytics-position', 'Body').attr('data-analytics-id', imageId).addClass('analytics--p-slideshow-image');
        });
        var slider = $(this).on('init afterChange', function (event, slick) {
          ...
          $('button', $currentDot).addClass('analytics--p-slideshow-current').removeClass('analytics--p-slideshow-left').removeClass('analytics--p-slideshow-right').attr('data-analytics-position', 'Body').attr('data-analytics-id', 'dot-current-' + carouselId);
          var $prevDot = $currentDot.prev();
          $('button', $prevDot).addClass('analytics--p-slideshow-left').removeClass('analytics--p-slideshow-current').removeClass('analytics--p-slideshow-right').attr('data-analytics-position', 'Body').attr('data-analytics-id', 'dot-prev-' + carouselId);
          var $nextDot = $currentDot.next();
          $('button', $nextDot).addClass('analytics--p-slideshow-right').removeClass('analytics--p-slideshow-current').removeClass('analytics--p-slideshow-left').attr('data-analytics-position', 'Body').attr('data-analytics-id', 'dot-next-' + carouselId);
        }).slick({
